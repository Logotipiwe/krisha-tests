...
func TestKek(t *testing.T) {
	...
	t.Run("For admin", func(t *testing.T) {
		...
		t.Run("Admin dashboard with some chats", func(t *testing.T) {
			...
			assert.True(t, strings.Contains(answers[0].Text, "Стандартный интервал: 1сек"))
			assert.True(t, strings.Contains(answers[0].Text, "Авто лимит: 54"))
			assert.True(t, strings.Contains(answers[0].Text, "10 - interval: 1, aps: 0 (TODO), explicit: false"))
			assert.True(t, strings.Contains(answers[0].Text, "11 - interval: 1, aps: 0 (TODO), explicit: false"))
			assert.True(t, strings.Contains(answers[0].Text, "12 - interval: 1, aps: 0 (TODO), explicit: false"))
		})
	...
	t.Run("For users with auto grant", func(t *testing.T) {
		...
		t.Run("Explicit grant accounted instead of auto (allow)", func(t *testing.T) {
			cleanupBeforeTest(t)
			client.SetAutoGrantLimit("10")
			client.SendUpdateFromOwner("/grant")
